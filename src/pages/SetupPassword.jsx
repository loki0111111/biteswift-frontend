...
export default function SetupPassword() {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        const response = await fetch(`https://biteswift-qw3s.onrender.com/api/auth/verify-token/${token}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://biteswift-qw3s.onrender.com/api/auth/setup-password", {
        method: "POST",
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 text-center max-w-md w-full">
          ...
          <p className="text-gray-500 text-sm leading-relaxed mb-6">
            Your password has been set. You can now log in to your BiteSwift dashboard.
          </p>
          <button
